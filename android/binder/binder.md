...
正如前面所说，跨进程通信是需要内核空间做支持的。传统的 IPC 机制如管道、Socket 都是内核的一部分，因此通过内核支持来实现进程间通信自然是没问题的。 Binder 是如何加入到内核中呢？这就得益于 Linux 的**动态内核可加载模块**（Loadable Kernel Module，LKM）的机制；模块是具有独立功能的程序，它可以被单独编译，但是不能独立运行。它在运行时被链接到内核作为内核的一部分运行。这样，Android 系统就可以通过动态添加一个内核模块运行在内核空间，用户进程之间通过这个内核模块作为桥梁来实现通信。
...
![Binder IPC 原理](D:\source\git\blog\AsideAndroid\images\binder\binder_memory_map.jpg)

## 四. Binder 通信模型

Binder使用Client-Server通信方式 ：一个进程作为Server提供诸如视频/音频解码，视频捕获，地址本查询，网络连接等服务；多个进程作为Client向Server发起服务请求，获得所需要的服务。要想实现Client-Server通信据必须实现以下两点：一是server必须有确定的访问接入点或者说地址来接受Client的请求，并且Client可以通过某种途径获知Server的地址；二是制定Command-Reply协议来传输数据。
...
![java_binder](D:\source\git\blog\AsideAndroid\images\binder\java_binder.jpg)
...
- framework层的ServiceManager类与Native层的功能并不完全对应，framework层的ServiceManager类的实现最终是通过BinderProxy传递给Native层来完成。

### 5.2 Binder类分层

整个Binder从kernel至，native，JNI，Framework层所涉及的全部类

![java_binder_framework](D:\source\git\blog\AsideAndroid\images\binder\java_binder_framework.jpg)

### 5.3 Binder通信实现

先简单说说大家熟悉的TCP/IP的五层通信体系结构:

![tcp_ip_arch](D:\source\git\blog\AsideAndroid\images\binder\tcp_ip_arch.jpg)
...
![binder_ipc_arch](D:\source\git\blog\AsideAndroid\images\binder\binder_ipc_arch.jpg)
...
![binder_dataflow.jpg](D:\source\git\blog\AsideAndroid\images\binder\binder_dataflow.jpg)
...
![binder_protocol](D:\source\git\blog\AsideAndroid\images\binder\binder_transaction_ipc.jpg)
...
http://baronzhang.com/blog/Android/%E5%86%99%E7%BB%99-Android-%E5%BA%94%E7%94%A8%E5%B7%A5%E7%A8%8B%E5%B8%88%E7%9A%84-Binder-%E5%8E%9F%E7%90%86%E5%89%96%E6%9E%90/#more